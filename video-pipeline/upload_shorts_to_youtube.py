...
UPLOAD_REPORT_PATH = os.path.join(REPORT_DIR, "shorts_upload_report.json")
...
def make_title(filename):
    ...
    if len(title) <= 92: title = title + " #Shorts"
    return title
...
def make_description(channel, title, script_path):
    ...
    parts.append(f"#Shorts #{tag_prefix} #{niche_tag} #2026")
...
def make_tags(channel, title):
    ...
    base_tags = ["Shorts", "YouTube Shorts"] + list(CHANNEL_TAGS.get(channel, []))
...
def main():
    ...
    for i, video_file in enumerate(pending, 1):
        ...
        # Skip thumbnail for Shorts (YouTube auto-generates)
        thumb_path = None
        ...
        if result == "quota_exceeded":
            ...
            print(f"  Uploaded: https://youtube.com/shorts/{vid_id} (status: {status})")
            ...
            try:
                share_to_facebook(title, f"https://youtube.com/shorts/{vid_id}", channel, is_short=True)
            except Exception:
                pass
        else:
